...
# Create a cursor, the cursor that does stuff to the database 
C = conn.cursor()
...
# Create table
C.execute("""CREATE TABLE addresses (first_name text, last_name text, addresss text, city text, state text, zipcode intege)""")'''
...
def edit():
    ...
    # Creat global variables for text names
...
def submit():
    ...
    # Insert into table
    C.execute("INSERT INTO addresses VALUES (:f_name, :l_name, :city, :address, :state, :zipcode)", 
       {'f_name': f_name.get(),
       'l_name': l_name.get(),
       'city': city.get(),
       'address': address.get(),
